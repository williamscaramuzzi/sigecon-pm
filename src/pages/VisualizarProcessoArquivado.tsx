...
import {
    ...
    DriveFolderUpload as DesarquivarIcon
...
import type { ContratoEmpenhado as ProcessoArquivado } from '../models/ProcessoCompra';
import type { EtapaProcesso } from '../models/EtapaProcesso';


const VisualizarProcessoArquivado: React.FC = () => {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const { userRole } = useAuth();
    // Estados para os dados do processo
    const [processo, setProcesso] = useState<ProcessoArquivado | null>(null);
    const [etapas, setEtapas] = useState<EtapaProcesso[]>([]);
    const [loading, setLoading] = useState<boolean>(true);
    const [editMode, setEditMode] = useState<Record<string, boolean>>({});
    const [idEtapaEditando, setIdEtapaEditando] = useState("")
    const [editValues, setEditValues] = useState<ProcessoArquivado | null>(null);
    ...
    const fetchProcessoData = async () => {
        ...
        try {
            // Buscar dados do processo
            const processoDoc = await getDoc(doc(db, "z_processos_concluidos", id));

            if (processoDoc.exists()) {
                const processoData = { ...processoDoc.data() } as ProcessoArquivado;
                setProcesso(processoData);
                setEditValues(processoData);

                // Buscar etapas do processo
                const etapasQuery = query(
                    collection(db, "z_processos_concluidos_etapas"),
    ...
    const handleEditField = (field: keyof ProcessoArquivado) => {
        setEditMode(prev => ({ ...prev, [field]: true }));
    };

    // Cancelar edição
    const handleCancelEdit = (field: keyof ProcessoArquivado) => {
    ...
    const handleChangeField = (field: keyof ProcessoArquivado, value: any) => {
        if (editValues) {
            setEditValues({
                ...editValues,
                [field]: value
    ...
    const handleSaveField = async (field: keyof ProcessoArquivado) => {
    ...
    const renderField = (label: string, field: keyof ProcessoArquivado, value: any, type: 'text' | 'number' = 'text') => {
        ...
        if (!value) displayValue = "Sem empenho"
    ...
    async function handleExcluirProcesso(e: any) {
        ...
        if (confirm("Tem certeza que deseja deletar o processo?")) {
            ...
            try {
                //o id é o nup
                const processoRef = doc(db, "z_processos_concluidos", processo.nup);

                // Exclui o processo
                await deleteDoc(processoRef)

                //Procura todas as etapas e exclui também
                const etapasRef = collection(db, "z_processos_concluidos_etapas")
                ...
                listaEtapasDoProcessoExcluido.forEach(async (etapa) => {
                    await deleteDoc(doc(db, "z_processos_concluidos_etapas", etapa.id))
                })
                setSnackbarMessage('Processo excluído com sucesso!');
                setSnackbarSeverity('success');
                setSnackbarOpen(true);

                navigate("/consultar_processos_arquivados")
            } catch (error) {
    ...
    async function handleDesarquivar(e: any) {
        if (!processo || !editValues || !processo.nup) return;
        let com_empenho = true;
        if (!processo.num_empenho) com_empenho = false
        let mensagem_do_confirm = `O processo tem número de empenho, e portanto será movido para a tabela de processos com contratos empenhados. Tem certeza que deseja desarquivar o Processo ${processo.nup}?`
        if (!com_empenho) mensagem_do_confirm = `O processo não tinha empenho, e portanto será movido para a tabela de processos em andamento. Tem certeza que deseja desarquivar o Processo ${processo.nup}?`
        const confirmou = confirm(mensagem_do_confirm)

        if (confirmou) {
            try {
                if (com_empenho) {
                    //copiar processo para tabela de processos com empenho, chamada no firebase de contratos empenhados
                    ...
                    etapas.forEach(async (currentEtapa) => {
                        //copia cada etapa para a tabela de etapas dos contratos com empenho
                        ...
                        //deleta cada etapa da tabela etapas dos contratos arquivados
                        const etapaADeletar = doc(db, "z_processos_concluidos_etapas", `${currentEtapa.nup}_${currentEtapa.data}`)
                        await deleteDoc(etapaADeletar)
                    })

                } else {
                    //copiar processo para tabela geral de processos que ainda não tinham emepnho
                    const novoDoc = doc(db, "processos", processo.nup)
                    await setDoc(novoDoc, processo)

                    //tratar as etapas do processo
                    etapas.forEach(async (currentEtapa) => {
                        //copia cada etapa para a tabela de etapas gerais dos processos sem empenho
                        const etapaconcluida = doc(db, "etapas", `${currentEtapa.nup}_${currentEtapa.data}`)
                        await setDoc(etapaconcluida, currentEtapa)

                        //deleta cada etapa da tabela etapas de processos arquivados
                        const etapaADeletar = doc(db, "z_processos_concluidos_etapas", `${currentEtapa.nup}_${currentEtapa.data}`)
                        await deleteDoc(etapaADeletar)
                ...
                //finalmente deleta o processo da tabela processos concluídos, pois agora ele está desarquivado na tabela adequada
                const processoRef = doc(db, "z_processos_concluidos", processo!.nup);
                await deleteDoc(processoRef)

                setSnackbarMessage('Processo desarquivado com sucesso!');
                setSnackbarSeverity('success');
                setSnackbarOpen(true);
    ...
    function renderEtapasRows(etapas: EtapaProcesso[], label: string, field: keyof ProcessoArquivado, value: any, type: 'text' | 'number' = 'text') {
        return (
            etapas.map(etapa => {
        ...
        <Box>
            <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', '@media print': { display: 'none' } }}>
                <Typography variant="h4" component="h1">
                    Ver processo arquivado
                </Typography>

                <Button
                    variant="outlined"
                    startIcon={<ArrowBackIcon />}
                    onClick={() => navigate('/consultar_processos_arquivados')}
                >
                    Voltar
                </Button>
            </Box>
                        ...
                        {userRole === "gerente" ? (
                            <Grid container size={{ xs: 12, md: 12 }} spacing={1} sx={{ '@media print': { display: 'none' } }}>
                                <Grid size={{ xs: 10, md: 3 }}>
                                    <Button variant="outlined" color="primary" sx={{ whiteSpace: 'nowrap' }} onClick={(e) => { handleDesarquivar(e) }}>
...
export default VisualizarProcessoArquivado;